...
def load_model_FSC(args, model_without_ddp):
    if args.resume:
        ...
        if 'patch_embed.proj.weight' in checkpoint['model'] and checkpoint['model']['patch_embed.proj.weight'].shape != model_without_ddp.state_dict()['patch_embed.proj.weight'].shape:
            print(f"Removing key pos_embed from pretrained checkpoint")
            del checkpoint['model']['patch_embed.proj.weight']

        # if 'patch_embed.proj.bias' in checkpoint['model'] and checkpoint['model']['patch_embed.proj.bias'].shape != model_without_ddp.state_dict()['patch_embed.proj.bias'].shape:
        #     print(f"Removing key pos_embed from pretrained checkpoint")
